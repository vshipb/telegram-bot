package io.qmbot.telegrambot.commandbot.commands.helpCommand;

import io.qmbot.telegrambot.commandbot.commands.IBotCommand;
import io.qmbot.telegrambot.commandbot.commands.ICommandRegistry;
	...
	public void execute(AbsSender absSender, Message message, String[] arguments) {
		if (ICommandRegistry.class.isInstance(absSender)) {
			...
			if (arguments.length > 0) {
				...
				try {
					absSender.execute(SendMessage.builder().chatId(message.getChat().getId()).text(reply).parseMode("HTML").build());
